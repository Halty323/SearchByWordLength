...
public class MainActivity extends AppCompatActivity {
    ...
    private View.OnClickListener listener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            int searchText = Integer.parseInt(search.getText().toString()); // считывание длины строки для поиска

            mapPoem = stringMapPoem(poem); // формирование мап подстрок поэмы

            String resultSearch = searchMapPoem(searchText, mapPoem); // поиск

            if (resultSearch == null) { // если в поэме ничего не найдено, то
                output.setText("Строки с такой длиной в поэме не найдено");
            } else {
    ...
    private String searchMapPoem(int lengthSearch, Map<Integer, String> map) {

        String result = "";
        int count = 0;

        for (Integer key: map.keySet()) { // перебор мап по ключам
            if(map.get(key).length() == lengthSearch) { // если в строке с ключом key имеется фраза поиска, то
        ...
        if (count == 0) {
            ...
            return count + " упоминаний строк с такой длиной в :" + result; // вывод количества найденных в поэме фраз и подстрок поэмы, где они были найдены
        }
    }
